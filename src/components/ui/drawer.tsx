...
>(({ className, children, showHandle = true, ...props }, ref) => (
  <DrawerPortal>
    ...
    <VaulDrawer.Content
      ref={ref}
      className={`fixed bottom-0 left-0 right-0 z-[61] mx-auto flex max-h-[calc(100dvh-env(safe-area-inset-top,0px)-0.5rem)] max-w-md flex-col rounded-t-3xl border border-zinc-200 bg-surface pt-[env(safe-area-inset-top,0px)] pb-[max(1rem,env(safe-area-inset-bottom))] outline-none ${className ?? ""}`}
      {...props}
    >
      {showHandle ? (
